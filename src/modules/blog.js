...
export const validate = async (folder, slug) => {
  ...
  if (!stat.isDirectory())
    throw Error(`Post must be within a directory named by slug (${slug})`);
  ...
  if (!post || !post.length)
    throw Error(`Unable to find ${slug}.md or index.md for ${slug}`);
...
export const copyAssets = async (folder, meta, publicDir) => {
  if (!meta || !meta.slug || !meta.assets || !publicDir)
    throw Error("Insufficient metadata to copy assets");

  // eslint-disable-next-line no-restricted-syntax
  for (const asset of meta.assets) {
    // eslint-disable-next-line no-await-in-loop
    await fs.copy(
      path.resolve(folder, meta.slug, asset),
      path.resolve(publicDir, folder, meta.slug, asset)
    );
  }
};
...
export const parse = async (folder, slug, siteRoot) => {
  ...
  const postData = fm(
    await fs.readFile(path.resolve(folder, slug, meta.index), "utf8")
  );
  ...
  if (!date || !title || !author)
    throw Error(`Insufficient frontmatter for ${slug}`);
